...
object Q3 {
  ...
  def longestNonUKCount(flightDataDS: Dataset[FlightData]) = {
    ...
    val nonUKCounts = ukFlights.groupBy(col("passengerId"), col("ukArrivals"))
      .agg(sum(expr(
        """case
          | when 'uk' not in (from,to) then 1
          | else 0
          |end""".stripMargin)).as("nonuk"))

    // Calculate the longest streak of non-UK flights for each passenger
    nonUKCounts.groupBy("passengerId")
